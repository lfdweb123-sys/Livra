...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadApplicationStatus() async {
    ...
    if (mounted) {
      setState(() {
        _vendorStatus = vendorSnap.docs.isNotEmpty
            ? vendorSnap.docs.first.data()['status']
            : null;
        _driverStatus = driverSnap.docs.isNotEmpty
            ? driverSnap.docs.first.data()['status']
            : null;
      });
    }
  }
  ...
  Future<void> _contactUs() async {
    final mailto = Uri(
        scheme: 'mailto', path: _supportEmail, query: 'subject=Support Livra');
    try {
  ...
  Future<void> _copyEmail() async {
    ...
    if (mounted)
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Email copié.')));
  }
  ...
  Widget _partnerButton(BuildContext context, String label, IconData icon,
      String route, String? status, String dashboardRoute,
      {bool blockedByOtherRole = false}) {
    final disabled =
        status == 'pending' || status == 'suspended' || blockedByOtherRole;
      ...
      child: DecoratedBox(
        ...
        child: Material(
          ...
          child: InkWell(
            ...
            onTap: disabled
                ? null
                : () => context.push(goToDashboard ? dashboardRoute : route),
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: Row(
                children: [
                  Icon(goToDashboard ? Icons.dashboard_rounded : icon,
                      size: 18,
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          goToDashboard ? 'Aller à mon espace' : label,
                          style: TextStyle(
                              color: disabled ? AppColors.textSecondary : null,
        ...
        child: ListView(
          ...
          children: [
            ...
            _partnerButton(
              ...
              '/driver/home',
              blockedByOtherRole:
                  _vendorStatus != null && _vendorStatus != 'rejected',
            ),
            const SizedBox(height: 12),
            _partnerButton(
              context,
              ...
              '/vendor/dashboard',
              blockedByOtherRole:
                  _driverStatus != null && _driverStatus != 'rejected',
            ),
            const SizedBox(height: 26),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 6),
                ...
                children: [
                  ...
                  if (_supportWhatsapp.isNotEmpty)
                    ListTile(
                      ...
                      subtitle: Text('+${_supportWhatsapp.replaceAll('+', '')}',
                          style: const TextStyle(fontSize: 12)),
                      onTap: () async {
                        final digits =
                            _supportWhatsapp.replaceAll('+', '').trim();
                        ...
                        try {
                          ...
                          if (!opened && mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content: Text(
                                      "Impossible d'ouvrir WhatsApp. Est-il installé ?")),
                            );
                          }
                        } catch (e) {
                          if (mounted) {
