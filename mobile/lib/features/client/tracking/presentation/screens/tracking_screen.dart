...
/// piloté par le stream Firestore drivers/{id}.
class TrackingScreen extends StatefulWidget {
  final String type; // order | ride
  final String id;
  TrackingScreen({super.key, required this.type, required this.id});
...
class _TrackingScreenState extends State<TrackingScreen> {
  ...
  void _listenDriver(String driverId) {
    ...
    _driverSub = _db.collection('drivers').doc(driverId).snapshots().listen((snap) {
      ...
      final newPos = LatLng((position['latitude'] as num).toDouble(), (position['longitude'] as num).toDouble());
      setState(() {
        ...
        children: [
            ...
            child: _driverPosition == null
                ...
                : TweenAnimationBuilder<double>(
                    ...
                    builder: (context, t, child) {
                      ...
                      return GoogleMap(
                        initialCameraPosition: CameraPosition(target: animatedPos, zoom: 15),
                        markers: {
                          Marker(markerId: MarkerId('driver'), position: animatedPos),
                        },
                      );
                    },
                  ),
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                Text(_statusLabels[_status] ?? _status, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                SizedBox(height: 4),
